...
That is what ChatGPT would answer to a 5-year-old kid.
Alright, let's imagine you have a little robot friend named Robo. Robo likes to explore and do different things, but Robo doesn't always know what to do next. A Markov Decision Process (MDP) is like giving Robo a set of rules to help it decide what to do next based on where it is and what it knows.
...
:::{#exm-roboClean}
A mobile robot has the job of collecting empty soda cans in an office environment. It
...
a small state set $\mathcal{S} = \{\texttt{high}, \texttt{low} \}$. In each state, the agent can 
decide whether to 
...
When the energy level is **high**, recharging would always be foolis
h, so we do not include it
in the action set for this state. The action sets are then 
...
(producing a low reward). 

If the energy level is **high**, then a period of active search can
...
$\alpha$ and reduces it to low with probability $1 - \alpha$. On the other hand, a period of searching undertaken
when the energy level is low leaves it low with probability $\beta$ and depletes the battery with
probability $1 - \beta$.  In the latter case, the robot must be rescued, and the battery is then
recharged back to **high**. Each can collected by the robot counts as a unit reward, whereas
a reward of $-3$ results whenever the robot has to be rescued. Let 
$r_{\texttt{search}}$ and $r_{\texttt{wait}}$, with $r_{\texttt{search}} > r_{\texttt{wait}}$,
denote the expected numbers of cans the robot will collect (and hence the
...
on a step in which the battery is depleted. This system is then a finite MDP, and we
...
digraph finite_state_machine {
    ...
    rankdir=LR;
    node [shape = point] search_0;
    node  [label =<search>, shape= point] search_1;
    node  [label = <wait>, shape = point] wait_0;
    node  [label = <wait>, shape = point] wait_1;
    node  [label = <recharge>, shape= point] recharge;
    node [
        label = <low>,
        fillcolor=gray,
        style="filled",
    ...
    low -> recharge [label = "1,0"];
	recharge -> high -> low;
	low -> wait_0 -> low [label = "1, r(wait) "];
    low -> search_0 -> low [label = "$\beta$ "];
    search_0 -> high [label = "$1 - \beta$ "];
    high -> wait_1 -> high [label = "1, $r_{wait}$"];
    high -> search_1 -> high [label = "1, $r_{wait}$"];
    search_1 -> search_1 [label = "< r<sub> search <\sub>>"];
    high -> search_1 -> low [label= "$1 - \alpha, r_{search}$"];
}
```
:::
...
:::{#exr-001}
Give a table analogous to that in [p.53, Ex. 3.3,@Sutton2018],
but for $p(s_0 , r |s, a)$. It should have columns for $s$, $a$, $s_0$ ,

...
::: {#fig-grid-world-exm fig-env="figure*"}
![Grid World example. Problem scheme , possible actions and sampling
reward](/assets/ch_04/grid_world_exm.png){.lightbox}

Grid World example. Problem scheme , possible actions and sampling reward
:::

::: {exm-grid_world}
The Bellman equation must hold for each state for the value function $v_{\pi}$
shown in @fig-grid-world-exm (right). Show numerically that this equation holds
for the center state, valued at +0.7, with respect to its four neighboring
states, valued at +2.3, +0.4, 0.4, and +0.7. (These numbers are accurate only to
one decimal place.)
:::
